...
(function() {
  ...
  var qzone = require('./qqauth-qzone');
  ...
  var QQBot = (function() {
    ...
    QQBot.prototype.save_group_member = function(group, info) {
      log.debug("STEP 7: SAVE GROUP MEMBER",group.name);
    ...
    QQBot.prototype.update_group_member = function(options, callback) {
      ...
      log.debug("STEP 3: BEFORE GET GROUP MEMBER",group.name);
      return this.api.get_group_member(group.code, this.auth, (function(_this) {
        log.debug("STEP 5: AFTER GET GROUP MEMBER",group.name);
        return function(ret, e) {
          log.debug("STEP 6: UPDATE GROUP MEMBER",group.name);
          log.debug("STEP 6: RET",ret.retcode);
          if (ret == null ) return ;
          if (ret.retcode === 0) {
    ...
    QQBot.prototype.update_all_group_member = function(callback) {
      ...
      results = [];
      log.debug("STEP 1: UPDATE ALL GROUP MEMBER");
      for (i = 0, len = groups.length; i < len; i++) {
        group = groups[i];
        log.debug("STEP 2." + (i+1) + ": BEFORE UPDATE GROUP MEMBER",group.name);
        results.push(this.update_group_member(group, function(ret, error) {
          log.debug("STEP 2." + (i+1) + ": AFTER UPDATE GROUP MEMBER");
          log.debug("STEP 2." + (i+1) + ": UPDATE RET",ret.retcode);
          finished += 1;
          successed += ret;
    ...
    QQBot.prototype.update_all_members = function(callback) {
      ...
      qzone.get_cardname(this.cookies,function(memberlist){
